        ...
        <div v-for="user in users" :key="user._id" class="user-card" @click="goToProfile(user._id)">
          <div class="user-info">
            <div class="avatar-container">
              <img
                :src="user.avatar ? `https://api.stchub.ru/uploads/avatars/${user.avatar}` : '/images/placeholders/avatar.png'"
  ...
  methods: {
    ...
    async loadUsers(page = 1) {
      try {
        ...
        const res = await axios.get(`https://api.stchub.ru/users?${queryString}`)
    ...
    async loadStatusCounts() {
      for (const status of this.statuses) {
        try {
          const response = await axios.get(`https://api.stchub.ru/users/count?status=${status.enTitle}`)
