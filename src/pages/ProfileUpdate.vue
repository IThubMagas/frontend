<template>
  <div class="flex justify-center">
    <div class="mainProfile w-fit flex mt-[40px] gap-[97px]">
      <div class="p-20px rounded-lg sticky top-[10px] mb-[60px] h-fit overflow-hidden">
        <ul class="flex gap-2 flex-col w-[234px]">
          <a :href="`#${item.id}`" @click="setActiveMenuItem(index)" v-for="(item, index) in menuItems" :key="item.id"
            class="py-4 pl-5 bg gap-y-1 bg-white text-sm font-semibold">
            <a :class="{ 'text-[#5E61FF]': activeMenuItem === index }">
              {{ item.title }}
            </a>
          </a>
        </ul>
      ...
      <div class="profile-main min-w-[896px]">
        ...
        <div class="profile-header" id="general">
          ...
          <div class="profile-avatar flex mt-8 items-center gap-6">
            <div class="relative">
              <img
                :src="avatarPreview || (UserData.avatar ? `https://api.stchub.ru/uploads/avatars/${UserData.avatar}` : 'images/placeholders/avatar.png')"
          ...
          <div class="flex gap-3 mt-8">
            <div class="data-lastname max-h-[76px] w-[33%]">
              ...
              <input v-model="UserData.lastName" class="w-[100%] max-h-12 mt-1 p-3 rounded-lg border" type="text"
            ...
            <div class="data-name max-h-[76px] w-[33%]">
              ...
              <input v-model="UserData.firstName" class="w-[100%] max-h-12 mt-1 p-3 rounded-lg border" type="text"
                placeholder="Введите имя">
            </div>
            <div class="data-surname max-h-[76px] w-[33%]">
              <p class="font-semibold">Отчество</p>
              <input v-model="UserData.patronymic" class="w-[100%] max-h-12 mt-1 p-3 rounded-lg border" type="text"
          ...
          <div class="flex gap-3 mt-8">
            <div class="max-h-[76px] w-[50%]">
              ...
              <input v-model="UserData.age" class="w-[100%] max-h-12 p-3 rounded-lg border" type="number"
                placeholder="Введите возраст">
            </div>
            <div class="max-h-[76px] w-[50%]">
              <p class="font-semibold">Населенный пункт</p>
              <input v-model="UserData.city" class="w-[100%] max-h-12 p-3 rounded-lg border" type="text"
          ...
          <div class="mt-8">
            ...
            <div class="flex gap-3">
              <div class="w-[50%]">
                ...
                <input v-model="UserData.phoneNumber" class="w-[100%] max-h-12 mt-2 p-3 rounded-lg border" type="text"
                  placeholder="+7 (928) 321 22 22" readonly>
              </div>
              <div class="w-[50%]">
                <p class="font-semibold">Почта</p>
                <input v-model="UserData.email" class="w-[100%] max-h-12 mt-2 p-3 rounded-lg border" type="text"
                  placeholder="name@mail.ru" readonly>
              </div>
            </div>
            <div v-if="hasContacts" class="mt-4 flex flex-wrap gap-2">
              <a v-if="UserData.contacts.telegram" :href="`https://t.me/${UserData.contacts.telegram}`" target="_blank"
                class="contact-link">
                Telegram
              </a>
              <a v-if="UserData.contacts.github" :href="`https://github.com/${UserData.contacts.github}`"
        ...
        <div class="mt-[60px]" id="aboutMe">
          ...
          <div>
            ...
            <textarea v-model="UserData.about" class="w-full h-[100px] p-3 mt-3 rounded-lg border"
        ...
        <div class="mt-[60px]">
          <div class="flex justify-between items-center mb-4">
            ...
            <button @click="openModal('achievement')"
              class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#e0e7ff] text-[#5E61FF] text-[14px] font-semibold rounded-lg border border-[#d0d7ff] hover:bg-[#d0d7ff] hover:-translate-y-0.5 transition-all duration-200">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                <path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2" />
              </svg>
              Добавить достижение
          ...
          <div class="flex flex-col gap-4">
            <div v-for="(achievement, index) in achievements" :key="index"
              class="border border-gray-300 rounded-lg p-4 hover:shadow-md transition-all duration-200 bg-white">
              <div class="flex justify-between items-start">
                <h3 class="text-base font-bold text-[#5E61FF] cursor-pointer w-fit"
                  @click="downloadFile(achievement.file)">
                  {{ achievement.text }}
                </h3>
                <div class="flex gap-2 ml-3">
                  <button @click="editItem('achievement', index)"
                  ...
                  <button @click="deleteItem('achievement', index)"
        ...
        <!-- Безопасность -->
        <div class="mt-[60px]" id="security">
          <p class="font-semibold text-[20px]">Безопасность</p>

          <div class="flex gap-3">
            <div>
              <p class="font-semibold mt-4">Старый пароль</p>
              <input v-model="UserData.oldPassword" class="min-w-[440px] max-h-12 mt-3 p-3 rounded-lg border"
                type="password" placeholder="********">
            </div>
            <div>
              <p class="font-semibold mt-4">Новый пароль</p>
              <input v-model="UserData.newPassword" class="min-w-[440px] max-h-12 mt-3 p-3 rounded-lg border"
                type="password" placeholder="********">
            </div>
          </div>
        </div>
        ...
        <!-- Уведомления -->
        <div class="mt-[60px]" id="notifications">
        ...
        <div class="gap-3 flex mt-[60px] mb-[60px]">
          <button class="py-3 px-6 min-w-[150px] bg-[#5E61FF] rounded-lg text-white font-semibold"
            @click="updateUser">Сохранить</button>
          <button class="py-3 px-6 min-w-[107px] bg-white rounded-lg font-semibold border"
            @click="resetProfile">Отмена</button>
        </div>
      </div>
    </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div v-else-if="modalType === 'project'" class="form-group">
            ...
            <input v-model="modalData.title" class="form-input"
              placeholder="Например: Разработка веб-сервиса VITRIN.JOBS" />
          ...
          <div v-else-if="modalType === 'social'" class="form-group">
            ...
            <input v-model="modalData.socialLink" type="url" class="form-input"
          ...
          <div v-else-if="modalType === 'contacts'" class="form-group">
            ...
            <input v-model="modalData.telegram" class="form-input" placeholder="Например: Test123" />
            <label class="form-label">GitHub Ник</label>
            <input v-model="modalData.github" class="form-input" placeholder="Например: Test123" />
          </div>
          ...
          <div v-else-if="modalType === 'language'" class="form-group">
            ...
            <select v-model="modalData.level" class="form-input">
              <option v-for="languageStatus in languageStatuses" :value="languageStatus.value">{{ languageStatus.text }}
              </option>
            </select>
          </div>
        </div>
...
async function loadUserProfile() {
  ...
  try {
    const response = await axios.get("https://api.stchub.ru/users/profile/me", {
...
async function updateUser() {
  ...
  try {
    ...
    const response = await axios.put(
      "https://api.stchub.ru/users/profile/me",
      formData,
      {
        headers: {
          ...getAuthHeaders(),
...
const downloadFile = async (file) => {
  ...
  if (typeof file === 'string') {
    if (!file.startsWith('http')) {
      file = `https://api.stchub.ru/uploads/achievements/${file}`;
    }
